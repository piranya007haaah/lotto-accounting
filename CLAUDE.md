...
npx tsx scripts/window-check.ts           # ตัด "ดูรายเดือน" ที่หน้าพอร์ตแล้วยอดยังบวกกันได้ทั้งปีไหม (646 จุด)
...
### เลือกช่วงที่ดู: ทั้งปี ↔ รายเดือน (`snapshot-window.ts`)

ชิปเหนือ KPI — **ดีฟอลต์คือเดือนล่าสุด** ที่มีข้อมูล (คำถามที่ถามทุกวันคือ "เดือนนี้เป็น
ยังไง" ไม่ใช่ยอดทั้งปี · ผู้ใช้ขอเอง ก.ย. 2569) · กด "🗓️ ทั้งปี" ได้ของเดิมทุกอย่าง

- `sliceSnapshot(snapshot, win, replay)` คืน **`PortfolioSnapshot` รูปเดิมเป๊ะ** ⇒ KPI/กราฟ/
  รายขา/ป๊อปอัป วาดต่อได้โดยไม่ต้องรู้ว่ากำลังดูเดือนหรือปี · **ไม่คำนวณ pnl ใหม่**
  แค่ตัดช่วงจากเส้นที่ engine คิดมาแล้ว + `computeRiskMetrics` ตัวเดียวกับ `computeSnapshot`
- ⚠️⚠️ **index ของเส้นทุนเลื่อนไป 1 จาก index ของผลหวย** — `curve[i]` = ทุนหลังจบวันที่
  `i−1` ⇒ ช่วงเส้นทุน `[a, b]` = ผลหวยวันที่ `a … b−1` · เผลอส่ง `[a, b]` ให้ตัวนับงวด
  จะนับวันแรกของเดือนถัดไปซ้ำ (วัดจริง: ถูก 1,643 แทน 1,588) — `scripts/window-check.ts`
  คือประตูที่กันไว้: กำไร/ถูก/แทงของทุกเดือนบวกกันต้องเท่ายอดทั้งปีเป๊ะทุกพอร์ตทุกขา
- ถูก/แทงรายเดือนนับด้วย `legWinRateIn()` ของ engine (กติกาเดียวกับทั้งปี: ข้ามวันหยุด
  และงวดที่ n=0) ⇒ ต้องมี `replay` · snapshot เก่าจาก Python ไม่มีข้อมูลดิบให้นับ
  **โชว์ "—" ไปตรง ๆ ห้ามเดา**
- โหมดเดือนเปลี่ยนความหมายของ 2 ช่อง: "ทุนเริ่มต้น" → **ทุนต้นเดือน** (ทุนที่วิ่งมาถึง) ·
  "เดือนที่ร่วงหนักสุด" → **วันที่ร่วงหนักสุด** (`worstDayIn` — ยอดวันเดียว ไม่ใช่ร่วงสะสม)
- บาร์ "กำไรรายเดือน" ใช้ **ทั้งปีเสมอ** แม้เจาะเดือนเดียว — มันคือบริบทว่าเดือนนี้ดี/แย่
  กว่าเดือนอื่นแค่ไหน · ป๊อปอัปรายขาเปิดมาที่เดือนเดียวกับที่เลือกไว้ข้างนอก
...
- ⚠️⚠️ **แต่ละใบตอบคำถามเดียว ห้ามเอายอดคนละช่วงมาปนกันในใบเดียว** — ใบที่ 1 =
  *งวดนี้เท่านั้น* · ใบ "วันนี้ถึงตอนนี้" = ทั้งวัน · ใบภาพรวมพอร์ต + ตารางรายวัน =
  เดือนนี้ (มีป้ายเดือนกำกับ) · เคยแอบใส่แถว "เดือนนี้" ลงในใบผลงวดเพื่อไม่ให้ใบโล่ง
  แล้วผู้ใช้อ่านไม่ออกว่าเลขไหนของวันไหน (ก.ย. 2569) — **ความโล่งแก้ด้วยอย่างอื่น
  ห้ามแก้ด้วยการยัดตัวเลขคนละช่วงเข้าไป**
...
- ⚠️ **จำนวนปีที่วัดผลได้ = จำนวนปีที่หวยตัวนั้นมี − 1 เสมอ** (ปีแรกไม่มีอดีตให้เทรน)
  · ตระกูล **แม่โขง / ลาวพลัส / ลาวพรีเมี่ยม** เพิ่งเก็บตั้งแต่ปี 68 ⇒ มี 68-69 = **วัดได้ปีเดียว**
  (26 กลุ่มจาก 270 เป็นแบบนี้ · อีก 4 กลุ่มมีปีเดียว = ทำ walk-forward ไม่ได้เลย)
  ⇒ หน้าจอ **ต้องบอกจำนวนปีที่มี + ช่วงปี** ไว้ใต้หัวข้อเสมอ และกรณีทำไม่ได้ต้องขึ้นเหตุผล
  ไม่ใช่ซ่อนทั้ง section เงียบ ๆ (ผู้ใช้ถามว่า "ทำไม walkforward แค่ปีเดียว" เพราะแบบนี้)
  · ปีในตัวเลือกด้านบนเป็นปีของ **ทั้งตาราง** ไม่ใช่ของหวยตัวนั้น — คนละเลขกัน
