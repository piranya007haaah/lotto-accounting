...
function wfBubble(input: FormulaCardInput, wf: WalkForwardResult, buttons: LineMessage[]): LineMessage {
  ...
  return bubble({
    // ⚠️ "วัดผล N ปี · จากข้อมูล N+1 ปี" — ปีแรกสุดใช้เทรนอย่างเดียวเสมอ ไม่มีอดีตให้เรียน
    //    หวยที่เพิ่งเปิด (ข้อมูล 2 ปี) จึงได้บรรทัดเดียว ซึ่งถูกแล้ว ไม่ใช่การ์ดพัง
    kicker: `วัดผล ${wf.folds.length} ปี จากข้อมูล ${wf.folds.length + 1} ปี · ไม่มีการมองอนาคต`,
    title: "ถ้าใช้สูตรนี้มาตลอด",
    when: "ทุกปีเทรนด้วยปีก่อนหน้าทั้งหมด แล้ววัดผลบนปีนั้น (ปีแรกใช้เทรนอย่างเดียว)",
    body: rows,
    footer: buttons,
  });
}
