...
function MonthEvidence({ score, entries, params, isTest, onBack }: {
  ...
  return <div className="space-y-3 text-[12px]">
    ...
    <div className="rounded-xl p-3" style={{ background: "var(--accent-tint)" }}>
      ...
      <p className="dim mt-1 text-[11px]">ขึ้นเดือนใหม่จึงสร้างชุดเลขใหม่ ไม่เปลี่ยนเลขระหว่างเดือน</p>
    </div>
    ...
    {tab === "draws" ? <>
      ...
      <p className="dim text-[11px]">กดวันที่เพื่อดูรายละเอียดต้นทุนและเงินรางวัล · วันที่ไม่มีผลไม่คิดเงิน</p>
      <div className="max-h-72 overflow-auto" role="region" aria-label="ผลรายงวด" tabIndex={0}>
        <table className="w-full text-left text-[11px]"><thead><tr><th>วันที่</th><th>ผล</th><th>ถูก/ไม่ถูก</th><th className="text-right">กำไร</th><th className="text-right">สะสม</th></tr></thead>
...
export function MonthWindowDetails({ row, entries, params, lottery, initialFormula, initialMonth, onClose }: {
  ...
  useEffect(() => { content.current?.parentElement?.scrollTo({ top: 0 }); }, [month, formula]);
  ...
  return <Modal title={`รายละเอียดกรอบ ${row.months} เดือน`} subtitle={`${lottery} · ${formula}`} onClose={onClose}>
    ...
    {score ? <MonthEvidence key={`${formula}-${month}`} score={score} entries={entries} params={params} isTest={month === "test"} onBack={() => setMonth(null)} /> : <div className="space-y-3 text-[12px]">
      ...
      {row.formulas.map((f, i) => <button key={f.formula} type="button" aria-pressed={formula === f.formula} className="w-full rounded-xl border p-2.5 text-left" style={{ background: formula === f.formula ? "var(--accent-tint)" : undefined, borderColor: "var(--line)" }} onClick={() => setFormula(f.formula)}>
      ...
      {current.folds.map((fold) => <button key={fold.month} type="button" className="row w-full py-2.5 text-left" onClick={() => setMonth(fold.month)}>
      ...
      <button type="button" className="w-full rounded-xl p-3 text-left" style={{ background: "var(--accent-tint)" }} onClick={() => setMonth("test")}>
