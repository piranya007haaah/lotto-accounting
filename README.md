...
จากตัวหนังสือด้วย Claude แล้วเก็บลง Supabase
...
LINE (rich menu / ลิงก์ LIFF)
  ...
  หน้าเว็บ Next.js (LIFF)  ── liff.getIDToken() ──┐
        ...
        ▼                                          ▼
  /api/ocr ─┬─ ถอด QR ตรวจสอบสลิป (ในเครื่อง)     │  ทุก API ตรวจ ID token
        │     └─ Claude (vision + structured output) │  กับ LINE ก่อนเสมอ
...
src/
  app/
    ...
    api/
      ...
      ocr/                   รับรูป → ถอด QR → เช็คซ้ำ → อัปโหลด → ให้ Claude อ่าน
  ...
  lib/
    ...
    ocr.ts                   prompt + schema + รวมค่าจาก QR กับที่โมเดลอ่านได้
...
### 4. Claude API (ใช้อ่านตัวหนังสือบนรูป)

ขอ API key จาก [Anthropic Console](https://console.anthropic.com/) แล้วใส่ `ANTHROPIC_API_KEY`

> ถ้าไม่ใส่ ระบบยังใช้ได้ปกติ และยังถอด **เลขที่รายการกับธนาคาร** จาก QR บนสลิปให้อยู่ดี
> เหลือแค่ยอดเงินและวันที่ที่ต้องพิมพ์เอง
...
ANTHROPIC_API_KEY=sk-... npx tsx scripts/ocr-smoke.ts ./slip.jpg   # QR + ให้โมเดลอ่านตัวหนังสือ
```
