...
| ค่าใช้จ่ายต่ำ | QR ฟรี, Vision ฟรี 1,000 ภาพ/เดือน, เรียก Claude เฉพาะใบที่อ่านไม่ครบเท่านั้น |
...
| ได้จาก QR แน่นอน | ต้องอ่านจากตัวหนังสือ (Claude) หรือกรอกเอง |
|---|---|
...
อ่านสามชั้นเรียงตามความน่าเชื่อถือและค่าใช้จ่าย ชั้นถัดไปทำงานเฉพาะตอนชั้นก่อนได้ค่าไม่ครบ
...
| 3. Claude | เฉพาะใบที่ชั้น 1–2 ยังได้ยอดเงินหรือวันที่ไม่ครบ | ตามจริง แทบไม่ถูกเรียก |

- ผลลัพธ์บอกไว้ใน `ocr.sources` ว่าผ่านชั้นไหนมาบ้าง เช่น `["qr", "vision"]`
- **QR ชนะเสมอ** สำหรับเลขที่รายการและธนาคาร ไม่ว่าชั้นอื่นจะอ่านได้อะไร
...
src/
  ...
  lib/
    ...
    ocr.ts                   ร้อยสามชั้นเข้าด้วยกัน + prompt/schema ของตัวสำรอง
...
### 5. Claude API (ไม่บังคับ — เป็นตัวสำรอง)

ใส่ `ANTHROPIC_API_KEY` ถ้าอยากให้มีตัวสำรองตอน Vision อ่านยอดเงินหรือวันที่ไม่ได้
ถ้าไม่ใส่ ระบบยังใช้ได้ปกติ แค่ใบที่อ่านไม่ครบจะให้กรอกเอง

> ถ้าตั้ง `OCR_MODEL` เป็นรุ่น Haiku ระบบจะไม่ส่ง `effort` ไปให้อัตโนมัติ เพราะรุ่นนั้นไม่รองรับ
